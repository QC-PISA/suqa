...
const char qxqcharmap[3] = {' ','Q','X'};

#ifdef SPARSE
void qxq_print(double *v_re, double *v_im, std::vector<uint> actives){
#else
void qxq_print(double *v_re, double *v_im, uint vecsize){
#endif
    ...
    for(size_t idx = 0; idx< state_norm.size(); ++idx){
        ...
        for(uint i=0U; i<9U; ++i){
            gms[idx][i] = qxqcharmap[(game_stidx & 3U)]; // 0 -> empty, 1-> first pl, 2-> second pl, 3-> inactive
            game_stidx>>=2U;
        }
    }
...
#define DEBUG_READ_STATE_QXQ() {\
    ...
    qxq_print((double*)host_state_re,(double*)host_state_im, suqa::state.size()); \
} 
#else  // NDEBUG
#define DEBUG_READ_STATE_QXQ()
#endif
...
#ifdef SPARSE
#define DEBUG_READ_STATE_QXQ() {\
    qxq_print((double*)suqa::state.data_re,(double*)suqa::state.data_im,suqa::actives); \
}
//    printf("vnorm = %.12lg\n",suqa::vnorm());
#else // NON SPARSE
#define DEBUG_READ_STATE_QXQ() {\
    qxq_print((double*)suqa::state.data_re,(double*)suqa::state.data_im, suqa::state.size()); \
}
#endif // end SPARSE
#endif // end GPU
...
bool double_turn(int pl) {
    ...
    Move move;
    cout << "\nPlayer "<<qxqcharmap[pl+1]<<", make move: " << flush;
